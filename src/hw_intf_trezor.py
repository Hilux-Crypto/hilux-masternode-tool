...
def json_to_tx(tx_json):
    ...
    if t.version == 3 and dip2_type != 0:
        ...
        t.extra_data = hilux_utils.num_to_varint(tx_json["extraPayloadSize"]) + bytes.fromhex(
...
class MyTxApiInsight(object):
    ...
    def __init__(self, hiluxd_inf, cache_dir):
        self.hiluxd_inf = hiluxd_inf
    ...
    def fetch_json(self, *path, **params):
        if path:
            if len(path) >= 2:
                if path[0] == 'tx':
                    try:
                        j = self.hiluxd_inf.getrawtransaction(path[1], 1, skip_cache=self.skip_cache)
                        return j
                    except Exception:
                        raise
                else:
...
def sign_tx(hw_session: HWSessionBase, rt_data: AppRuntimeData, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    insight_network = 'insight_hilux'
    if rt_data.is_testnet:
        insight_network += '_testnet'
    hilux_network = rt_data.hilux_network

    tx_api = MyTxApiInsight(rt_data.hiluxd_intf, rt_data.tx_cache_dir)
    ...
    for utxo_index, utxo in enumerate(utxos_to_spend):
        ...
        address_n = hilux_utils.bip32_path_string_to_n(utxo.bip32_path)
    ...
    for out in tx_outputs:
        ...
        if out.address[0] in hilux_utils.get_chain_params(hilux_network).B58_PREFIXES_SCRIPT_ADDRESS:
            stype = trezor_proto.OutputScriptType.PAYTOSCRIPTHASH
            logging.debug('Transaction type: PAYTOSCRIPTHASH' + str(stype))
        elif out.address[0] in hilux_utils.get_chain_params(hilux_network).B58_PREFIXES_PUBKEY_ADDRESS:
        ...
        if out.bip32_path:
            address_n = hilux_utils.bip32_path_string_to_n(out.bip32_path)
        else:
            address_n = None
...
def sign_message(hw_client, hw_coin_name: str, bip32path: str, message: str):
    address_n = hilux_utils.bip32_path_string_to_n(bip32path)
    try:
