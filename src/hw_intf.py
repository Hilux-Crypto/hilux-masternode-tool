...
from hilux_utils import bip32_path_n_to_string
...
def ping_device(hw_device: HWDevice, message: str):
    def ledger_ping(ctrl):
        ...
        try:
            ledger.sign_message(hw_device.hw_client, hilux_utils.get_default_bip32_path('MAINNET'), message, None)
        except CancelException:
            pass
...
class HwSessionInfo(HWSessionBase):
    ...
    def initiate_hw_session(self):
        ...
        def get_session_info_trezor(get_public_node_fun, hw_device_):
            ...
            path_ = hilux_utils.get_default_bip32_base_path(self.__runtime_data.hilux_network)
            path_n = hilux_utils.bip32_path_string_to_n(path_)
        ...
        try:
            if hw_device.hw_type == HWType.trezor:
                ...
                path = hilux_utils.get_default_bip32_base_path(self.__runtime_data.hilux_network)
        ...
        if not self.hw_client:
            ...
            try:
                try:
                    ...
                    if self.__runtime_data.hilux_network == 'TESTNET':
                        # check if Hilux testnet is supported by this hardware wallet
                        found_testnet_support = False
                        if self.hw_type in (HWType.trezor, HWType.keepkey):
                            try:
                                path = hilux_utils.get_default_bip32_base_path(self.__runtime_data.hilux_network)
                                path += "/0'/0/0"
                                path_n = hilux_utils.bip32_path_string_to_n(path)
                                addr = get_address(self, path_n, False)
                                if addr and hilux_utils.validate_address(addr, self.__runtime_data.hilux_network):
                            ...
                            addr = get_address(self, hilux_utils.get_default_bip32_path(
                                self.__runtime_data.hilux_network))
                            if hilux_utils.validate_address(addr, self.__runtime_data.hilux_network):
                                found_testnet_support = False

                        if not found_testnet_support:
                            url = get_note_url('DMT0002')
                            msg = f'Your hardware wallet device does not support Hilux TESTNET ' \
...
def sign_tx(hw_session: HwSessionInfo, utxos_to_spend: List[UtxoType], tx_outputs: List[TxOutputType], tx_fee):
    ...
    :param tx_outputs: destination addresses. Fields: 0: dest Hilux address. 1: the output value in satoshis,
    ...
    def _get_address(ctrl):
        ...
        if client:
            ...
            if hw_session.hw_type == HWType.trezor:
                ...
                try:
                    if isinstance(bip32_path, str):
                        bip32_path = hilux_utils.bip32_path_string_to_n(bip32_path)
