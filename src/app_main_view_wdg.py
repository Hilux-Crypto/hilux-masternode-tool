...
from hiluxd_intf import HiluxdInterface, Masternode
...
class WdgAppMainView(QWidget, QDetectThemeChange, ui_app_main_view_wdg.Ui_WdgAppMainView):
    ...
    def __init__(self, parent, app_config: AppConfig, hiluxd_intf: HiluxdInterface, hw_session: hw_intf.HwSessionInfo):
        QWidget.__init__(self, parent)
        QDetectThemeChange.__init__(self)
        self.current_view = Pages.PAGE_MASTERNODE_LIST
        self.main_window = parent
        self.app_config = app_config
        self.hiluxd_intf = hiluxd_intf
        ...
        self.wdg_masternode = WdgMasternodeDetails(self, self.app_config, self.hiluxd_intf, self.hw_session)
        self.last_hilux_price_usd = None
        self.last_hilux_price_fetch_ts = 0
        self.setupUi(self)
    ...
    def on_lblMnStatus_linkActivated(self, link: str):
        if self.cur_masternode:
            ...
            if link == 'copy_protx_to_config':
                ...
                cl.setText(self.cur_masternode.get_dmn_owner_public_address(self.app_config.hilux_network))
                ...
                cl.setText(self.cur_masternode.get_dmn_voting_public_address(self.app_config.hilux_network))
    ...
    def refresh_network_data(self):
        ...
        if self.app_config.show_hilux_value_in_fiat and (self.app_config.is_mainnet or SCREENSHOT_MODE):
            if not self.refresh_price_thread_ref and \
                    int(time.time()) - self.last_hilux_price_fetch_ts >= DASH_PRICE_FETCH_INTERVAL_SECONDS:
    ...
    def fetch_governance_info(self):
        ...
        _ginfo = self.hiluxd_intf.getgovernanceinfo()
        cur_block_height = self.hiluxd_intf.getblockcount()
        ...
        last_superblock_ts = self.hiluxd_intf.get_block_timestamp(gi.last_superblock)
        gi.next_superblock_ts = 0
        if 0 < cur_block_height <= gi.next_superblock:
            gi.next_superblock_ts = self.hiluxd_intf.get_block_timestamp(cur_block_height) + (
        ...
        gi.budget_available = float(self.hiluxd_intf.getsuperblockbudget(gi.next_superblock))
        ...
        # masternodes
        mns = self.hiluxd_intf.masternodes
        ...
        bi = self.hiluxd_intf.getblockchaininfo()
        gi.blockchain_size_on_disk = bi.get('size_on_disk')
        gi.blocks = bi.get('blocks')

        gi.last_block_ts = self.hiluxd_intf.get_block_timestamp(cur_block_height)
        gi.loaded = True
    ...
    def get_mn_protx(self, masternode: MasternodeConfig, protx_list_registered: List[Dict]) -> Optional[Dict]:
        ...
        if masternode.dmn_tx_hash:
            try:
                protx = self.hiluxd_intf.protx('info', masternode.dmn_tx_hash)
                if protx:
            ...
            if not protx:
                try:
                    ...
                    tx = self.hiluxd_intf.getrawtransaction(masternode.dmn_tx_hash, 1, skip_cache=True)
        ...
        if not (protx_state and ((protx_state.get('service') == masternode.ip + ':' + masternode.port) or
            ...
            try:
                if not protx_list_registered:
                    protx_list_registered.extend(self.hiluxd_intf.protx('list', 'registered', True))
    ...
    def refresh_status_thread(self, ctrl):
        ...
        try:
            ...
            self.hiluxd_intf.get_masternodelist('json', data_max_age=cache_max_age, protx_data_max_age=cache_max_age,
                                               feedback_fun=check_finishing)
            check_finishing()

            block_height = self.hiluxd_intf.getblockcount()
            check_finishing()
            ...
            for mn in mns:
                ...
                if not ms:
                    ms = MasternodeStatus(self.app_config.hilux_network)
                ...
                if collateral_id:
                    mn_info = self.hiluxd_intf.masternodes_by_ident.get(collateral_id)
                elif ip_port:
                    mn_info = self.hiluxd_intf.masternodes_by_ip_port.get(ip_port)
                else:
                    mn_info = None
            ...
            try:
                ...
                self.hiluxd_intf.fetch_mempool_txes(check_finishing)
            ...
            check_finishing()
            self.network_status.mempool_entries_count = len(self.hiluxd_intf.mempool_txes)
            ...
            for mn in mns:
                ...
                if int(time.time()) - ms.last_addr_balance_fetch_ts >= MN_BALANCE_FETCH_INTERVAL_SECONDS:
                    if not ms.collateral_address_mismatch and mn.collateral_address:
                        try:
                            coll_bal = self.hiluxd_intf.getaddressbalance([mn.collateral_address])
                    ...
                    if mn_info.protx and mn_info.protx.payout_address:
                        try:
                            ...
                            payout_bal = self.hiluxd_intf.getaddressbalance([mn_info.protx.payout_address])
                    ...
                    if mn_info.protx and mn_info.protx.operator_payout_address:
                        try:
                            ...
                            if mn_info.protx.operator_payout_address != mn_info.protx.payout_address:
                                payout_bal = self.hiluxd_intf.getaddressbalance([mn_info.protx.operator_payout_address])
                ...
                if mn_info.protx and mn_info.protx.last_paid_height and mn_info.protx.last_paid_height > 0:
                    ...
                    if not prev_last_paid_block or prev_last_paid_block != mn_info.protx.last_paid_height:
                        ...
                        if not ms.last_paid_ts:
                            ms.last_paid_ts = self.hiluxd_intf.get_block_timestamp(ms.last_paid_block)
                ...
                if self.hiluxd_intf.is_protx_update_pending(mn_info.protx_hash, mn_info.ip_port):
            ...
            if self.app_config.show_hilux_value_in_fiat and (self.app_config.is_mainnet or SCREENSHOT_MODE):
                ...
                if r:
                    ...
                    if r:
                        ...
                        if c:
                            v = float(c[0])
                            self.last_hilux_price_usd = v

            self.last_hilux_price_fetch_ts = int(time.time())
        except Exception as e:
            log.info('Error when fetching Hilux price: ' + str(e))

        finally:
            self.refresh_price_thread_ref = None

    def get_hilux_amount_str(self, amount: float, show_hilux_part: bool, show_fiat_part: bool,
                            show_hilux_lbl: bool = False, show_fiat_lbl: bool = False) -> str:
        if amount is not None:
            ...
            if (self.app_config.is_mainnet or SCREENSHOT_MODE) and self.last_hilux_price_usd is not None and \
                    self.app_config.show_hilux_value_in_fiat and show_fiat_part:
                if ret_str:
                    ret_str += ' / '
                ret_str += app_utils.to_string(round(amount * self.last_hilux_price_usd, 2))
                if show_fiat_lbl:
                    ret_str += ' USD'
        else:
            ret_str = ''
    ...
    def update_info_page(self):
        ...
        try:
            ...
            status = (
                ...
                f'<tr><td class="title">Budget available</td><td class="value">{app_utils.to_string(round(gi.budget_available, 2)) + " Hilux" if gi.loaded else "?"}</td></tr>'
            ...
            if (self.app_config.is_mainnet or SCREENSHOT_MODE) and self.app_config.show_hilux_value_in_fiat:
                price_str = app_utils.to_string(self.last_hilux_price_usd) if self.last_hilux_price_usd is not None \
                    else '?'
                status += f'<tr><td class="title">Hilux price (Kraken)</td><td class="value">{price_str} USD</td></tr>'
        ...
        if mn:
            ...
            if st:
                ...
                add_status_line('Payout addr. balance',
                                self.get_hilux_amount_str(st.payout_addr_balance, True, True, True, True))
                ...
                if mn.collateral_address.strip() and mn.collateral_address.strip() != st.payout_address:
                    ...
                    add_status_line('Collateral addr. balance',
                                    self.get_hilux_amount_str(st.collateral_addr_balance, True, True, True, True))
                ...
                if not st.protx_conf_pending:
                    ...
                    if st.owner_public_address_mismatch:
                        warnings.append(
                            ...
                            f'{short_address_str(mn.get_dmn_owner_public_address(self.app_config.hilux_network), 6)} '
                    ...
                    if st.voting_public_address_mismatch:
                        warnings.append(
                            ...
                            f'{short_address_str(mn.get_dmn_voting_public_address(self.app_config.hilux_network), 6)} '
...
class MasternodesTableModel(ExtSortFilterItemModel):
    def __init__(self, parent, masternodes: List[MasternodeConfig],
                 ...
                 get_hilux_amount_str_fun: Callable[[float, bool, bool, Optional[bool], Optional[bool]], str]):
        ExtSortFilterItemModel.__init__(self, parent, [
            ...
            TableModelColumn('collateral_addr_balance_hilux', 'Collateral balance [DASH]', False, 100,
                             horizontal_alignment=HorizontalAlignment.RIGHT),
            TableModelColumn('collateral_addr_balance_fiat', 'Collateral balance [USD]', False, 100,
                             horizontal_alignment=HorizontalAlignment.RIGHT),
            TableModelColumn('payout_addr_balance_hilux', 'Payout balance [DASH]', False, 100,
                             horizontal_alignment=HorizontalAlignment.RIGHT),
        ...
        self.get_hilux_amount_str = get_hilux_amount_str_fun
    ...
    def get_cell_value(self, row_idx: int, col_idx: int, for_sorting: bool):
        ...
        if row_idx < len(self.masternodes):
            ...
            if col:
                ...
                if col_name == 'no':
                ...
                elif col_name == 'collateral_addr_balance_hilux':
                    if st:
                        if for_sorting:
                            ...
                            ret_val = self.get_hilux_amount_str(st.collateral_addr_balance, True, False, False, False)
                    ...
                    if st:
                        if for_sorting:
                            ...
                            ret_val = self.get_hilux_amount_str(st.collateral_addr_balance, False, True, False, False)
                elif col_name == 'payout_addr_balance_hilux':
                    if st:
                        if for_sorting:
                            ...
                            ret_val = self.get_hilux_amount_str(st.payout_addr_balance, True, False, False, False)
                    ...
                    if st:
                        if for_sorting:
                            ...
                            ret_val = self.get_hilux_amount_str(st.payout_addr_balance, False, True, False, False)
        return ret_val
...
class MasternodeStatus:
    def __init__(self, hilux_network):
    ...
    def check_mismatch(self, masternode_cfg: MasternodeConfig, masternode_info: Masternode):
        ...
        if masternode_cfg.dmn_user_roles & DMN_ROLE_OWNER:
            ...
            owner_address_cfg = masternode_cfg.get_dmn_owner_public_address(self.hilux_network)
        ...
        if masternode_cfg.dmn_user_roles & DMN_ROLE_VOTING:
            voting_address_cfg = masternode_cfg.get_dmn_voting_public_address(self.hilux_network)
