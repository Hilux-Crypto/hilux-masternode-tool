...
from hiluxd_intf import HiluxdIndexException, Masternode
...
# will display data from cache, instead of requesting them from a hilux daemon, which is
# more time consuming.
...
class VotingMasternode(AttrsProtected):
    def __init__(self, masternode, masternode_config):
        ...
        :param masternode: ref to an object storing mn information read from the network (hiluxd_intf.Masternode)
    ...
    def apply_values(self, masternodes, last_superblock_time, next_superblock_datetime):
        ...
        try:
            ...
            if not self.get_value('title'):
                # if title value is not set (it's an external attribute, from hiluxcentral) then copy value from the
                # name column
...
class ProposalsDlg(QDialog, wnd_utils.QDetectThemeChange, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, hiluxd_intf):
        ...
        for idx, mn in enumerate(self.app_config.masternodes):
            ...
            if mn_ident not in mn_idents:
                if mn.dmn_voting_key_type == InputKeyType.PRIVATE:
                    voting_key = mn.get_current_key_for_voting(self.app_config, self.hiluxd_intf)
                    if voting_key:
                        if hilux_utils.validate_wif_privkey(voting_key, self.app_config.hilux_network):
    ...
    def display_budget_summary(self):
        ...
        if self.next_superblock_time and self.next_voting_deadline:
            ...
            if self.next_budget_approved is not None:
                budget_approved = f'<td><b>Budget approved:</b> {app_utils.to_string(round(self.next_budget_approved))} Hilux '
            ...
            if self.next_budget_requested is not None:
                budget_requested = f'<td><b>Budget requested:</b> {app_utils.to_string(round(self.next_budget_requested))} Hilux '
            ...
            if self.votes_loaded:
                if self.next_budget_approved_by_user_yes_votes is not None:
                    budget_approved_user_yes = \
                        ...
                        f'{app_utils.to_string(round(self.next_budget_approved_by_user_yes_votes))} Hilux '
                    if self.next_budget_amount:
                        budget_approved_user_yes += \
            ...
            message = '<html><head></head><style>td{padding-right:10px;white-space:nowrap;}</style><body>' \
                      ...
                      f'{bra}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} Hilux</td>' \
                      f'<td><b>Budget left:</b> {app_utils.to_string(round(self.next_budget_left))} Hilux</td></tr>' \
    ...
    def read_proposals_from_network(self):
        """ Reads proposals from the Hilux network. """
        ...
        try:
            ...
            log.info('Reading proposals from the Hilux network.')
            begin_time = time.time()
            proposals_new = self.hiluxd_intf.gobject("list", "valid", "proposals")
            ...
            if len(proposals_new) > 0:
                if errors < len(proposals_new)/10:
                    try:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # hiluxd sometimes does not return some proposals, so they are deactivated id the db
                        ...
                        # delete proposals which no longer exists in tha Hilux network
                        rows_removed = False
                ...
                log.warning('No proposals returned from hiluxd.')
            ...
            log.exception('Exception wile reading proposals from Hilux network.')
            self.display_message('')
            self.error_msg('Error while reading proposals data from the Hilux network: ' + str(e))
            raise
    ...
    def read_governance_data(self):
        try:
            ...
            self.governanceinfo = self.hiluxd_intf.getgovernanceinfo()
            ...
            self.next_budget_amount = float(self.hiluxd_intf.getsuperblockbudget(self.next_superblock))

            # superblocks occur every 16616 blocks (approximately 28.8 days)
            self.cur_block_height = self.hiluxd_intf.getblockcount()
            self.cur_block_timestamp = int(time.time())

            self.last_superblock_time = self.hiluxd_intf.get_block_timestamp(self.last_superblock)
            self.next_superblock_time = 0
            if 0 < self.cur_block_height <= self.next_superblock:
                self.next_superblock_time = self.hiluxd_intf.get_block_timestamp(self.cur_block_height) + (self.next_superblock - self.cur_block_height) * 2.5 * 60
            ...
            self.error_msg("Couldn't read governance info from the Hilux network. "
    ...
    def find_superblocks_for_timestamp(self, timestamp: int) -> Tuple[int, int]:
        ...
        def get_next_sb_ts(sb_nr: int) -> int:
            if sb_nr > self.last_superblock:
                ...
                sb_timestamp = self.hiluxd_intf.get_block_timestamp(sb_nr)
            return sb_timestamp
        ...
        if timestamp >= self.last_superblock_time:
            ...
            sb_before = self.last_superblock - (sb_cycles_diff * self.superblock_cycle_blocks)
            sb_before_ts = self.hiluxd_intf.get_block_timestamp(sb_before)
    ...
    def read_data_thread(self, ctrl):
        ...
        try:
            self.display_message('Connecting to Hilux daemon, please wait...')
            if not self.hiluxd_intf.open():
                self.error_msg('Hilux daemon not connected')
            else:
                try:
                    ...
                    mns = self.hiluxd_intf.get_masternodelist('json')
                    ...
                    if self.db_intf.db_active:
                        try:
                            ...
                            for row in cur.fetchall():
                                ...
                                dc_url_assumed = 'https://www.hiluxcentral.org/p/' + prop_name
                                if prop.ext_attributes_loaded:
                                    if not row[26] and not row[27] and time.time() - ext_attributes_load_time > 86400:
                                        ...
                                        prop.ext_attributes_loaded = False
                                    elif re.match(r'.*hiluxcentral\.org', prop_url.lower()) and \
                                        prop_url.lower() != dc_url_assumed.lower():
                                        # possibly invalid HiluxCentral URL entered by the proposal owner; refetch it
                                        # from the HiluxCentral API, but not too often
                ...
                except HiluxdIndexException as e:
            ...
            if not self.finishing:
                # read additional data from external sources, if configured (HiluxCentral)
                proposals = []
    ...
    def read_external_attributes(self, proposals):
        """Method reads additional proposal attributes from an external source such as HiluxCentral.org/HiluxNexus.org
        ...
        try:
            dc_api_url = self.app_config.hilux_central_proposal_api
            if dc_api_url:
                ...
                for idx, prop in enumerate(proposals):
                    ...
                    try:
                        ...
                        if p is not None:
                            ...
                            # dc_url_assumed = 'https://www.hiluxcentral.org/p/' +
                            if re.match(r'.*hiluxcentral\.org', cur_prop_url.lower()) and dw_url and \
                                    cur_prop_url != dw_url:
                                n = prop.get_value('name')
                                log.info(f'Fixing the possibly broken url related to HiluxCentral for the proposal '
                                         f'"{n}" (hash: {hash})')
    ...
    def read_voting_from_network(self, force_reload_all, proposals: List[Proposal]):
        """
        Retrieve from a Hilux daemon voting results for all defined masternodes, for all visible Proposals.
                    ...
                    try:
                        ...
                        for row_idx, prop in enumerate(proposals):
                            try:
                                ...
                                tm_begin = time.time()
                                try:
                                    votes = self.hiluxd_intf.rpc_call(False, False, 'gobject', 'getcurrentvotes',
                                                                     prop.get_value('hash'))
                                except Exception:
    ...
    def refresh_proposals_thread(self, ctrl):
        ...
        proposals = []
        if self.app_config.read_proposals_external_attributes:
            # select proposals for which we read additional data from external sources as HiluxCentral.org
            for prop in self.proposals:
                prop_url = prop.get_value('url')
                prop_name = prop.get_value('name')
                dc_url_assumed = 'https://www.hiluxcentral.org/p/' + prop_name
                if not prop.ext_attributes_loaded or \
                        (re.match(r'.*hiluxcentral\.org', prop_url.lower()) and  # Force re-fetch of data from HiluxCentral
    ...
    def refresh_details_tabs(self):
        try:
            ...
            if self.current_proposal:
                ...
                if not re.match(r'.*hiluxcentral\.org', url.lower()):
                    dc_url = 'https://www.hiluxcentral.org/p/' + prop.get_value('name')
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding" style="white-space:nowrap"><span>{app_utils.to_string(prop.get_value('payment_amount'))} Hilux&#47;cycle ({cycles_str}, {app_utils.to_string(prop.get_value('payment_amount_total'))} Hilux total)
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding">{app_utils.to_string(total_amount_requested)} Hilux{total_pct_requested}</td>
        </tr>
        <tr class="main-row">
            <td class="first-col-label">Approved from the next budget:</td>
            <td class="padding">{app_utils.to_string(total_amount_approved)} Hilux{total_pct_approved}</td>
        </tr>
    </tbody>
</table>
 </body>
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_key = mn_info.masternode_config.get_current_key_for_voting(self.app_config, self.hiluxd_intf)

                    if self.app_config.is_testnet:
                        vote_sig = hilux_utils.ecdsa_sign(serialize_for_sig, vote_key, self.app_config.hilux_network)
                    else:
                        vote_sig = hilux_utils.ecdsa_sign(serialize_for_sig, vote_key, self.app_config.hilux_network)

                    step = 3
                    v_res = self.hiluxd_intf.voteraw(
                    ...
                    if step in (1, 4):
                        ...
                        log.info('masternode_pub_key: %s' %
                                     str(hilux_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.hiluxd_intf))))
                        log.info('masternode_pub_key_hash: %s' %
                                     str(hilux_utils.pubkey_to_address(hilux_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.hiluxd_intf)), self.app_config.hilux_network)))
