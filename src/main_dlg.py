...
from hiluxd_intf import HiluxdInterface, HiluxdIndexException
...
class MainWindow(QMainWindow, QDetectThemeChange, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_dir, internal_ui_dark_mode_activated: bool = False):
        ...
        self.hiluxd_intf = HiluxdInterface(window=None,
        ...
        self.app_rt_data = AppRuntimeData(self.app_config, self.hiluxd_intf)
        ...
        self.is_hiluxd_syncing = False
        self.hiluxd_connection_ok = False
        self.connecting_to_hiluxd = False
    ...
    def setupUi(self, main_dlg: QMainWindow):
        ...
        self.main_view = WdgAppMainView(self, self.app_config, self.hiluxd_intf, self.hw_session)
    ...
    def load_configuration_from_file(self, file_name: Optional[str], ask_save_changes = True,
        ...
        try:
            hilux_network_sav = self.app_config.hilux_network
            ...
            if not self.hiluxd_intf.initialized:
                self.hiluxd_intf.initialize(self.app_config)
            self.editing_enabled = False
            self.configuration_to_ui()
            self.hiluxd_intf.reload_configuration()
            ...
            if file_name:
                ...
                self.update_config_files_mru_menu_items()
                if hilux_network_sav != self.app_config.hilux_network:
    ...
    def display_window_title(self):
        ...
        if self.app_config.hilux_network == 'TESTNET':
    ...
    def get_project_config_params_thread(self, ctrl, force_check):
        ...
        try:
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/Bertrand256/hilux-masternode-tool/master/app-params.json',
    ...
    def show_connection_initiated(self):
        """Shows status information related to a initiated process of connection to a hilux RPC. """
        self.set_status_text1('<b>RPC network status:</b> trying %s...' % self.hiluxd_intf.get_active_conn_description())
    ...
    def show_connection_failed(self):
        ...
        self.set_status_text1('<b>RPC network status:</b> failed connection to %s' % self.hiluxd_intf.get_active_conn_description(), 'error')

    def show_connection_successful(self):
        """Shows status information after successful connection to a Hilux RPC node."""
        self.set_status_text1('<b>RPC network status:</b> OK (%s)' % self.hiluxd_intf.get_active_conn_description(), 'success')

    def show_connection_disconnected(self):
        """Shows status message related to disconnection from Hilux RPC node."""
        self.set_status_text1('<b>RPC network status:</b> not connected')

    def connect_hilux_network(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        Connects do hilux daemon if not connected before and returns if it was successful.
        ...
        def wait_for_synch_finished_thread(ctrl):
            """
            Thread waiting for hilux daemon to finish synchronizing.
            """
            mtx = QMutex()
            cond = QWaitCondition()
            try:
                ...
                while not ctrl.finish:
                    synced = self.hiluxd_intf.issynchronized()
                    if synced:
                        self.is_hiluxd_syncing = False
                        self.show_connection_successful()
                        break
                    mnsync = self.hiluxd_intf.mnsync()
                    self.add_app_message(
                        DispMessage.DASH_NET_CONNECTION,
                        'Hiluxd is synchronizing: AssetID: %s, AssetName: %s' %
                ...
                mtx.unlock()
                self.wait_for_hiluxd_synced_thread = None

        def connect_thread(ctrl):
            """
            Test connection to hilux network inside a thread to avoid blocking GUI.
            :param ctrl: control structure to communicate with WorkerThread object (not used here)
            """
            try:
                synced = self.hiluxd_intf.issynchronized()
                self.hiluxd_intf.getblockchaininfo(verify_node=True)
                self.hiluxd_connection_ok = True
                if not synced:
                    logging.info("hiluxd not synced")
                    if not self.is_hiluxd_syncing and not (hasattr(self, 'wait_for_hiluxd_synced_thread') and
                                                                  self.wait_for_hiluxd_synced_thread is not None):
                        self.is_hiluxd_syncing = True
                        self.wait_for_hiluxd_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
        ...
        def connect_finished():
            ...
            self.connecting_to_hiluxd = False
            self.app_config.read_hilux_network_app_params(self.hiluxd_intf)
        ...
        if self.app_config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_hiluxd_synced_thread') and self.wait_for_hiluxd_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for hiluxd to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_hiluxd = True
    ...
    def on_action_check_network_connection_triggered(self):
        try:
            self.test_hilux_network_connection()
        except Exception as e:
            self.error_msg(str(e), True)

    def test_hilux_network_connection(self):
        def connection_test_finished():

            self.action_check_network_connection.setEnabled(True)
            self.action_hw_wallet.setEnabled(True)

            if self.hiluxd_connection_ok:
                self.show_connection_successful()
                if self.is_hiluxd_syncing:
                    self.info_msg('Connection successful, but Hilux daemon is synchronizing.')
                else:
                    self.info_msg('Connection successful.')
            else:
                if self.hiluxd_intf.last_error_message:
                    self.error_msg('Connection error: ' + self.hiluxd_intf.last_error_message)
                else:
        ...
        if self.app_config.is_config_complete():
            ...
            self.connect_hilux_network(call_on_check_finished=connection_test_finished)
        else:
    ...
    def on_action_open_proposals_window_triggered(self):
        try:
            ui = ProposalsDlg(self, self.hiluxd_intf)
            ui.exec_()
        except Exception as e:
            self.error_msg(str(e), True)
    ...
    def on_action_register_masternode_triggered(self, enabled):
        ...
        def on_proregtx_finished(masternode: MasternodeConfig):
            ...
            try:
                if cur_masternode.dmn_tx_hash != reg_dlg.dmn_reg_tx_hash or \
                    ...
                    self.hiluxd_intf.reset_masternode_data_cache()
        ...
        if cur_masternode:
            try:
                reg_dlg = reg_masternode_dlg.RegMasternodeDlg(self, self.app_config, self.hiluxd_intf, cur_masternode,
    ...
    def update_registrar(self, show_upd_payout: bool, show_upd_operator: bool, show_upd_voting: bool):
        ...
        if self.main_view.get_cur_masternode():
            upd_dlg = upd_mn_registrar_dlg.UpdMnRegistrarDlg(
                self, self.app_config, self.hiluxd_intf, self.main_view.get_cur_masternode(),
