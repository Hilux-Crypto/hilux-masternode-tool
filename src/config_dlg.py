...
import app_cache
from app_config import AppConfig, HiluxNetworkConnectionCfg
from hiluxd_intf import HiluxdInterface, control_rpc_call
...
class ConfigDlg(QDialog, Ui_ConfigDlg, WndUtils):
    def __init__(self, parent, app_config: AppConfig):
        ...
        # list of connections from self.local_config.hilux_net_configs split on separate lists for mainnet and testnet
        self.connections_mainnet = []
        self.connections_testnet = []
        self.connections_current = None
        self.current_network_cfg : Optional[HiluxNetworkConnectionCfg] = None
    ...
    def setupUi(self, dialog: QDialog):
        ...
        if self.local_config.is_mainnet:
            self.cboHiluxNetwork.setCurrentIndex(0)
            self.connections_current = self.connections_mainnet
        else:
            self.cboHiluxNetwork.setCurrentIndex(1)
            self.connections_current = self.connections_testnet
        for cfg in self.local_config.hilux_net_configs:
            if cfg.testnet:
        ...
        self.chbShowHiluxFIATValue.setChecked(self.local_config.show_hilux_value_in_fiat)
        ...
        if len(self.local_config.hilux_net_configs):
    ...
    @pyqtSlot(int)
    def on_cboHiluxNetwork_currentIndexChanged(self, index):
        """Executed after changing configuration between MAINNET and TESTNET."""
        if not self.disable_cfg_update:
            if index == 0:
                self.connections_current = self.connections_mainnet
                self.local_config.hilux_network = 'MAINNET'
            else:
                self.connections_current = self.connections_testnet
                self.local_config.hilux_network = 'TESTNET'
            self.display_connection_list()
            self.set_modified()
            self.lstConns.setCurrentRow(0)
        ...
        try:
            ...
            if isinstance(conns, list) and len(conns):
                ...
                if self.query_dlg('Do you really want to import connection(s) from clipboard?',
                    ...
                    self.local_config.hilux_net_configs.clear()
                    self.local_config.hilux_net_configs.extend(self.connections_mainnet)
                    self.local_config.hilux_net_configs.extend(self.connections_testnet)

                    added, updated = self.local_config.import_connections(
                        conns, force_import=True, limit_to_network=self.local_config.hilux_network)
                    for cfg in added:
                        cfg.enabled = True
    ...
    def on_btnRestoreDefault_clicked(self, enabled):
        if self.query_dlg('Do you really want to restore default connection(s)?',
                          buttons=QMessageBox.Yes | QMessageBox.Cancel,
                          default_button=QMessageBox.Yes, icon=QMessageBox.Information) == QMessageBox.Yes:
            cfgs = self.local_config.decode_connections(default_config.hiluxd_default_connections)
            if cfgs:
                # update the main list containing connections configuration from separate  lists dedicated
                # to mainnet and testnet - it'll be used by the import_connection method
                self.local_config.hilux_net_configs.clear()
                ...
                added, updated = self.local_config.import_connections(
                    cfgs, force_import=True, limit_to_network=self.local_config.hilux_network)
                self.connections_current.extend(added)
                if added or updated:
                    row_selected = self.lstConns.currentRow()
                    self.display_connection_list()
    ...
    def on_action_new_connection_triggered(self):
        ...
        cfg.testnet = True if self.cboHiluxNetwork.currentIndex() == 1 else False
    ...
    def on_chbRandomConn_toggled(self, checked):
        if not self.disable_cfg_update:
            self.local_config.random_hilux_net_config = checked
            self.set_modified()
    ...
    def update_connection_details_ui(self):
        ...
        try:
            ...
            self.chbRandomConn.setChecked(self.local_config.random_hilux_net_config)
        finally:
    ...
    @pyqtSlot(bool)
    def on_chbShowHiluxFIATValue_toggled(self, checked):
        self.local_config.show_hilux_value_in_fiat = checked
        self.set_modified()
    ...
    def on_btnSshReadRpcConfig_clicked(self):
        """Read the configuration of a remote RPC node from the node's hilux.conf file."""
        if self.current_network_cfg:
            ...
            if not ok or not username:
                return
            from hiluxd_intf import HiluxdSSH
            ssh = HiluxdSSH(host, int(port), username, auth_method=auth_method, private_key_path=private_key_path)
            try:
                if ssh.connect():
                    hiluxd_conf = ssh.find_hiluxd_config()
                    self.disable_cfg_update = True
                    if isinstance(hiluxd_conf, tuple) and len(hiluxd_conf) >= 3:
                        if not hiluxd_conf[0]:
                            self.info_msg('Remore Hilux daemon seems to be shut down')
                        elif not hiluxd_conf[1]:
                            self.info_msg('Could not find remote hiluxd.conf file')
                        else:
                            file = hiluxd_conf[2]
                            ...
                            if file.get('server', '1') == '0':
                                self.warn_msg("Remote hilux.conf parameter 'server' is set to '0', so RPC interface will "
                                             "not work.")
                            if not rpcuser:
                                self.warn_msg("Remote hilux.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                             "not work.")
                            if not rpcpassword:
                                self.warn_msg("Remote hilux.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                             "not work.")
                        self.update_connection_details_ui()
                    elif isinstance(hiluxd_conf, str):
                        self.warn_msg("Couldn't read remote hiluxd configuration file due the following error: " +
                                      hiluxd_conf)
                    ssh.disconnect()
            except Exception as e:
                self.error_msg(str(e), True)
    ...
    def on_btnTestConnection_clicked(self):
        if self.current_network_cfg:
            ...
            hiluxd_intf = HiluxdInterface(window=self)
            hiluxd_intf.initialize(self.local_config, connection=self.current_network_cfg,
                                  for_testing_connections_only=True)
            try:
                info = hiluxd_intf.getblockchaininfo(verify_node=True)
                if info:
                    try:
                        ret = hiluxd_intf.rpc_call(True, False, "checkfeaturesupport", "enhanced_proxy")
                    except Exception as e:
                        ret = None
