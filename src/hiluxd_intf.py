...
log = logging.getLogger('dmt.hiluxd_intf')
...
class HiluxdSSH(object):
    ...
    def find_hiluxd_config(self):
        """
        Try to read configuration of remote hilux daemon. In particular we need parameters concerning rpc
        configuration.
        :return: tuple (hiluxd_running, hiluxd_config_found, hiluxd config file contents as dict)
                or error string in error occurred.
        """
        hiluxd_running = False
        hiluxd_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find hiluxd process id if running
            try:
                pids = self.remote_command('ps -C "hiluxd" -o pid')
            except UnknownError:
                raise Exception('is hiluxd running on the remote machine?')
            pid = None
            ...
            if pid:
                hiluxd_running = True
                # using hiluxd pid find its executable path and then .hiluxcore directory and finally hilux.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        hiluxd_dir = os.path.dirname(executable)
                        hilux_conf_file = hiluxd_dir + '/.hiluxcore/hilux.conf'
                        conf_lines = []
                        try:
                            conf_lines = self.remote_command('cat ' + hilux_conf_file)
                        except Exception as e:
                            # probably error no such file or directory
                            # try to read hiluxd's cwd + cmdline
                            cwd_lines = self.remote_command('ls -l /proc/' + str(pid) + '/cwd')
                            if cwd_lines:
                                elems = cwd_lines[0].split('->')
                                if len(elems) >= 2:
                                    cwd = elems[1]
                                    hilux_conf_file = cwd + '/.hiluxcore/hilux.conf'
                                    try:
                                        conf_lines = self.remote_command('cat ' + hilux_conf_file)
                                    except Exception as e:
                                        # second method did not suceed, so assume, that conf file is located
                                        # i /home/<username>/.hiluxcore directory
                                        hilux_conf_file = '/home/' + self.username + '/.hiluxcore/hilux.conf'
                                        conf_lines = self.remote_command('cat ' + hilux_conf_file)
                        ...
                        hiluxd_config_found = True
            return hiluxd_running, hiluxd_config_found, config
        except Exception as e:
            return str(e)
...
class HiluxdIndexException(JSONRPCException):
    """
    Exception for notifying, that hilux daemon should have indexing option tuned on
    """
    def __init__(self, parent_exception):
        JSONRPCException.__init__(self, parent_exception.error)
        self.message = self.message + \
                       '\n\nMake sure the hilux daemon you are connecting to has the following options enabled in ' \
                       'its hilux.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute hiluxd with "-reindex" option (linux: ./hiluxd -reindex)'
                    ...
                    try:
                        try:
                            ...
                            if last_conn_reset_time:
                                raise HiluxdConnectionError(e)  # switch to another config if possible
                            else:
                                last_exception = e
                            ...
                            # exceptions raised most likely by not functioning hiluxd node; try to switch to another node
                            # if there is any in the config
                            log.warning('Error while calling of "' + str(func) + ' (3)". Details: ' + str(e))
                            raise HiluxdConnectionError(e)
...
class HiluxdInterface(WndUtils):
    def __init__(self, window,
        ...
        self.cur_conn_def: Optional['HiluxNetworkConnectionCfg'] = None
    ...
    def switch_to_next_config(self):
        """
        If there is another hiluxd config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to hilux RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to Hilux network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning hiluxd node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
    ...
    def open_internal(self):
        """
        Try to establish connection to hilux RPC daemon for current connection config.
        ...
        if not self.active:
            ...
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                if self.ssh is None:
                    self.ssh = HiluxdSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.app_config.is_testnet and not node_under_testnet:
                    raise Exception('This RPC node works under Hilux MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.app_config.is_mainnet and node_under_testnet:
                    raise Exception('This RPC node works under Hilux TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def reset_masternode_data_cache(self):
        cache_item_name = 'ProtxLastReadTime_' + self.app_config.hilux_network
        app_cache.set_value(cache_item_name, 0)
        cache_item_name = f'MasternodesLastReadTime_{self.app_config.hilux_network}'
        app_cache.set_value(cache_item_name, 0)
    ...
    def get_masternodelist(self, *args, data_max_age=MASTERNODES_CACHE_VALID_SECONDS,
        ...
        """
        Returns masternode list, read from the Hilux network or from the internal cache.
        ...
        if self.open():
            if len(args) == 1 and args[0] == 'json':
                last_read_time = app_cache.get_value(f'MasternodesLastReadTime_{self.app_config.hilux_network}', 0, int)
                if self.masternodes and data_max_age > 0 and int(time.time()) - last_read_time < data_max_age:
                    ...
                    app_cache.set_value(f'MasternodesLastReadTime_{self.app_config.hilux_network}', int(time.time()))
