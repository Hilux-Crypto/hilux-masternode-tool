...
from hilux_utils import bip32_path_string_to_n, bip32_path_n_to_string, pubkey_to_address
...
class Bip44Entry(object):
    ...
    def evaluate_address_if_null(self, db_cursor, hilux_network: str):
        if not self.address and self.xpub:
            key = self.get_bip32key()
            self.address = pubkey_to_address(key.PublicKey().hex(), hilux_network)
            if self.id:
