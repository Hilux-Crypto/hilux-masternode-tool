...
class SendFundsDestinationItem(QObject):
    ...
    def display_second_unit_value(self):
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            if self.value_amount is not None:
                self.lbl_second_unit_value.setText(app_utils.to_string(round(self.value_amount, 8)) + ' Hilux')
            else:
                self.lbl_second_unit_value.setText('')
    ...
    def validate(self):
        ...
        if not address:
            valid = False
        elif not hilux_utils.validate_address(address.strip(), self.app_config.hilux_network):
            valid = False
        else:
            self.message = None
        if valid:
...
class SendFundsDestination(QtWidgets.QWidget, WndUtils):
    ...
    def setupUi(self, Form):
        ...
        self.lbl_fee.setText('Fee [Hilux]')
    ...
    def use_all_funds_for_address(self, address_item):
        ...
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            # in this mode, due to the pct -> hilux conversion for each of the outputs, there can be left a reminder,
    ...
    def calculate_the_change(self) -> float:
        """Returns the change value in Hilux."""
        sum = 0.0
        for idx, addr in enumerate(self.recipients):
    ...
    def display_totals(self):
        ...
        text = f'<span class="label"><b>Total value of the selected inputs:</b>&nbsp;</span><span class="value">&nbsp;{self.inputs_total_amount} Hilux&nbsp;</span>'
        ...
        try:
            ...
            for line_idx, line in enumerate(data.split('\n')):
                if line:
                    ...
                    address_valid = hilux_utils.validate_address(address, hilux_network=None)
    ...
    def paint(self, painter, option: QStyleOptionViewItem, index: QModelIndex):
        if index.isValid():
            ...
            if mn.address.balance is not None:
                balance_str = 'Balance: ' + app_utils.to_string(mn.address.balance / 1e8) + ' Hilux'
            else:
        ...
        if index.isValid():
            ...
            if isinstance(data, Bip44AccountType):
                ...
                if data.balance is not None:
                    balance_str = 'Balance: ' + app_utils.to_string(data.balance / 1e8) + ' Hilux'
                else:
                    balance_str = 'Balance: unknown'
                painter.drawText(r, Qt.AlignLeft, balance_str)
