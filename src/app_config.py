...
class AppFeatureStatus(QObject):
    ...
    #  6: value read from the Hilux network (the highest priority by default)
    PRIORITY_DEFAULT = 0
...
class AppConfig(QObject):
    ...
    def __init__(self, ui_dark_mode_activated: bool):
        ...
        # List of Hilux network configurations. Multiple conn configs advantage is to give the possibility to use
        # another config if particular one is not functioning (when using "public" RPC service, it could be node's
        # maintenance)
        self.hilux_net_configs = []

        # to distribute the load evenly over "public" RPC services, we choose random connection (from enabled ones)
        # if it is set to False, connections will be used according to its order in hilux_net_configs list
        self.random_hilux_net_config = True

        # list of all enabled hiluxd configurations (HiluxNetworkConnectionCfg) - they will be used according to
        # the order in list
        self.active_hilux_net_configs = []

        # list of misbehaving hilux network configurations - they will have the lowest priority during next
        # connections
        ...
        self.hilux_central_proposal_api: str = ''
        self.hilux_nexus_proposal_api: str = ''

        # public RPC connection configurations
        self.public_conns_mainnet: Dict[str, HiluxNetworkConnectionCfg] = {}
        self.public_conns_testnet: Dict[str, HiluxNetworkConnectionCfg] = {}
        ...
        self.show_hilux_value_in_fiat = True
        ...
        try:
            self.default_rpc_connections = self.decode_connections(default_config.hiluxd_default_connections)
        except Exception:
    ...
    def save_cache_settings(self):
        if self.feature_register_dmn_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_REGISTER_DMN_AUTOMATIC_' + self.hilux_network,
                                self.feature_register_dmn_automatic.get_value())
        if self.feature_update_registrar_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_UPDATE_REGISTRAR_AUTOMATIC_' + self.hilux_network,
                                self.feature_update_registrar_automatic.get_value())
        if self.feature_update_service_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_UPDATE_SERVICE_AUTOMATIC_' + self.hilux_network,
                                self.feature_update_service_automatic.get_value())
        if self.feature_revoke_operator_automatic.get_value() is not None:
            app_cache.set_value('FEATURE_REVOKE_OPERATOR_AUTOMATIC_' + self.hilux_network,
                                self.feature_revoke_operator_automatic.get_value())

    def restore_cache_settings(self):
        ena = app_cache.get_value('FEATURE_REGISTER_AUTOMATIC_DMN_' + self.hilux_network, True, bool)
        self.feature_register_dmn_automatic.set_value(ena, AppFeatureStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_UPDATE_REGISTRAR_AUTOMATIC_' + self.hilux_network, True, bool)
        self.feature_update_registrar_automatic.set_value(ena, AppFeatureStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_UPDATE_SERVICE_AUTOMATIC_' + self.hilux_network, True, bool)
        self.feature_update_service_automatic.set_value(ena, AppFeatureStatus.PRIORITY_APP_CACHE)
        ena = app_cache.get_value('FEATURE_REVOKE_OPERATOR_AUTOMATIC_' + self.hilux_network, True, bool)
        self.feature_revoke_operator_automatic.set_value(ena, AppFeatureStatus.PRIORITY_APP_CACHE)

    def copy_from(self, src_config):
        self.hilux_network = src_config.hilux_network
        self.hilux_net_configs = copy.deepcopy(src_config.hilux_net_configs)
        self.random_hilux_net_config = src_config.random_hilux_net_config
        ...
        self.hilux_central_proposal_api = src_config.hilux_central_proposal_api
        ...
        self.show_hilux_value_in_fiat = src_config.show_hilux_value_in_fiat
    ...
    def configure_cache(self):
        ...
        if not os.path.exists(self.tx_cache_dir):
            ...
            if self.is_testnet:
                ...
                try:
                    for file in glob.glob(os.path.join(self.cache_dir, 'insight_hilux_testnet*.json')):
    ...
    def reset_configuration(self):
        ...
        :return:
        """
        self.hilux_net_configs.clear()
        self.active_hilux_net_configs.clear()
        self.defective_net_configs.clear()
        self.masternodes.clear()
        self.random_hilux_net_config = True
        self._remote_app_params.clear()
        self._hilux_blockchain_info.clear()
        self.public_conns_mainnet.clear()
        self.public_conns_testnet.clear()
        self.hw_keepkey_psw_encoding = 'NFC'
        self.hilux_network = 'MAINNET'
        self.block_explorer_tx_mainnet = 'https://insight.hilux.org/insight/tx/%TXID%'
        self.block_explorer_addr_mainnet = 'https://insight.hilux.org/insight/address/%ADDRESS%'
        self.block_explorer_tx_testnet = 'https://testnet-insight.hiluxevo.org/insight/tx/%TXID%'
        self.block_explorer_addr_testnet = 'https://testnet-insight.hiluxevo.org/insight/address/%ADDRESS%'
        self.tx_api_url_mainnet = 'https://insight.hilux.org/insight'
        self.tx_api_url_testnet = 'https://testnet-insight.hiluxevo.org/insight'
        self.hilux_central_proposal_api = 'https://www.hiluxcentral.org/api/v1/proposal?hash=%HASH%'
        self.hilux_nexus_proposal_api = 'https://api.hiluxnexus.org/proposals/%HASH%'
        self.check_for_updates = True
        self.backup_config_file = True
    ...
    def read_from_file(self, hw_session: 'HwSessionInfo', file_name: Optional[str] = None,
        ...
        if os.path.exists(file_name):
            ...
            try:
                ...
                hilux_network = config.get(section, 'hilux_network', fallback='MAINNET')
                if hilux_network not in ('MAINNET', 'TESTNET'):
                    logging.warning(f'Invalid hilux_network value: {hilux_network}')
                    hilux_network = 'MAINNET'
                self.hilux_network = hilux_network
                ...
                self.random_hilux_net_config = self.value_to_bool(config.get(section, 'random_hilux_net_config',
                                                                            fallback='1'))
                ...
                self.show_hilux_value_in_fiat = self.value_to_bool(
                    config.get(section, 'show_hilux_value_in_fiat', fallback='1'))
                ...
                for section in config.sections():
                    try:
                        if re.match('MN\d', section):
                            ...
                            cfg = HiluxNetworkConnectionCfg('rpc')
                            ...
                            if not skip_adding:
                                self.hilux_net_configs.append(cfg)
    ...
    def save_to_file(self, hw_session: 'HwSessionInfo', file_name: Optional[str] = None,
        ...
        config.set(section, 'hilux_network', self.hilux_network)
        if self.__hw_type:
        ...
        config.set(section, 'random_hilux_net_config', '1' if self.random_hilux_net_config else '0')
        ...
        config.set(section, 'show_hilux_value_in_fiat', '1' if self.show_hilux_value_in_fiat else '0')
        ...
        # save hilux network connections
        for idx, cfg in enumerate(self.hilux_net_configs):
    ...
    def apply_remote_app_params(self):
        def get_feature_config_remote(symbol) -> Tuple[Optional[bool], Optional[int], Optional[str]]:
            ...
            if features:
                ...
                if feature:
                    a = feature.get(self.hilux_network.lower())
                    if a:
    ...
    def read_hilux_network_app_params(self, hiluxd_intf):
        """ Read parameters having impact on the app's behavior (sporks/dips) from the Hilux network. Called
    ...
    def get_default_protocol(self) -> int:
        prot = None
        if self._remote_app_params:
            dp = self._remote_app_params.get('defaultHiluxdProtocol')
            if dp:
                prot = dp.get(self.hilux_network.lower())
        return prot
    ...
    def is_config_complete(self):
        for cfg in self.hilux_net_configs:
            if cfg.enabled:
                return True
        return False
    ...
    def prepare_conn_list(self):
        """
        Prepare list of enabled connections for connecting to hilux network. 
        :return: list of HiluxNetworkConnectionCfg objects order randomly (random_hilux_net_config == True) or according 
        ...
        for cfg in self.hilux_net_configs:
            if cfg.enabled and self.is_testnet == cfg.testnet:
                tmp_list.append(cfg)
        if self.random_hilux_net_config:
            ordered_list = []
            while len(tmp_list):
                idx = randint(0, len(tmp_list)-1)
                ordered_list.append(tmp_list[idx])
                del tmp_list[idx]
            self.active_hilux_net_configs = ordered_list
        else:
            self.active_hilux_net_configs = tmp_list

    def get_ordered_conn_list(self):
        if not self.active_hilux_net_configs:
            self.prepare_conn_list()
        return self.active_hilux_net_configs
    ...
    def decode_connections(self, raw_conn_list) -> List['HiluxNetworkConnectionCfg']:
        """
        Decodes list of dicts describing connection to a list of HiluxNetworkConnectionCfg objects.
        :param raw_conn_list: 
    ...
    def decode_connections_json(self, conns_json):
        ...
        :return: list of HiluxNetworkConnectionCfg objects or None if there was an error while importing
        """
        try:
    ...
    def import_connections(self, in_conns, force_import, limit_to_network: Optional[str]):
        ...
        :param in_conns: list of HiluxNetworkConnectionCfg objects.
        ...
        if in_conns:
            ...
            for conn in self.hilux_net_configs:
                if conn.testnet:
            ...
            for nc in in_conns:
                if (self.hilux_network == 'MAINNET' and nc.testnet == False) or \
                   (self.hilux_network == 'TESTNET' and nc.testnet == True) or not limit_to_network:
                    id = nc.get_conn_id()
                    ...
                    if not conn:
                        if force_import or not app_cache.get_value('imported_default_conn_' + nc.get_conn_id(),
                           ...
                           (testnet_conn_count == 0 and nc.testnet) or  (mainnet_conn_count == 0 and nc.mainnet):
                            ...
                            self.hilux_net_configs.append(nc)
    ...
    def get_conn_cfg_by_id(self, id):
        """
        Returns HiluxNetworkConnectionCfg object by its identifier or None if does not exists.
        :param id: Identifier of the sought connection.
        :return: HiluxNetworkConnectionCfg object or None if does not exists.
        """
        for conn in self.hilux_net_configs:
    ...
    def is_testnet(self) -> bool:
        return self.hilux_network == 'TESTNET'

    @property
    def is_mainnet(self) -> bool:
        return self.hilux_network == 'MAINNET'

    @property
    def hw_coin_name(self):
        if self.is_testnet:
            return 'Hilux Testnet'
        else:
            return 'Hilux'

    def get_block_explorer_tx(self):
        if self.hilux_network == 'MAINNET':
...
class MasternodeConfig:
    ...
    def get_current_key_for_voting(self, app_config: AppConfig, hiluxd_intf) -> str:
        return self.dmn_voting_private_key

    def get_dmn_owner_public_address(self, hilux_network) -> Optional[str]:
        if self.__dmn_owner_key_type == InputKeyType.PRIVATE:
            if self.__dmn_owner_private_key:
                try:
                    address = hilux_utils.wif_privkey_to_address(self.__dmn_owner_private_key, hilux_network)
                except Exception as e:
                    logging.exception(str(e))
                    address = ''
                return address
    ...
    def get_dmn_owner_pubkey_hash(self) -> Optional[str]:
        if self.dmn_owner_key_type == InputKeyType.PRIVATE:
            if self.__dmn_owner_private_key:
                try:
                    pubkey = hilux_utils.wif_privkey_to_pubkey(self.__dmn_owner_private_key)
            ...
            if self.__dmn_owner_address:
                ret = hilux_utils.address_to_pubkey_hash(self.__dmn_owner_address)
                if ret:
                    return ret.hex()
        return ''

    def get_dmn_voting_public_address(self, hilux_network) -> Optional[str]:
        if self.__dmn_voting_key_type == InputKeyType.PRIVATE:
            if self.__dmn_voting_private_key:
                try:
                    address = hilux_utils.wif_privkey_to_address(self.__dmn_voting_private_key, hilux_network)
                except Exception as e:
                    logging.exception(str(e))
                    address = ''
                return address
    ...
    def get_dmn_voting_pubkey_hash(self) -> Optional[str]:
        if self.__dmn_voting_key_type == InputKeyType.PRIVATE:
            if self.__dmn_voting_private_key:
                try:
                    pubkey = hilux_utils.wif_privkey_to_pubkey(self.__dmn_voting_private_key)
                    pubkey_bin = bytes.fromhex(pubkey)
                    pub_hash = bitcoin.bin_hash160(pubkey_bin)
                    pub_hash = pub_hash.hex()
                except Exception as e:
            ...
            if self.__dmn_voting_address:
                ret = hilux_utils.address_to_pubkey_hash(self.__dmn_voting_address)
                if ret:
                    return ret.hex()
        return ''
    ...
    def get_dmn_operator_pubkey(self) -> Optional[str]:
        if self.__dmn_operator_key_type == InputKeyType.PRIVATE:
            if self.__dmn_operator_private_key:
                try:
                    pubkey = hilux_utils.bls_privkey_to_pubkey(self.__dmn_operator_private_key)
                except Exception as e:
                    logging.exception(str(e))
                    pubkey = ''
                return pubkey
...
class HiluxNetworkConnectionCfg(object):
    ...
    def identical(self, cfg2):
        ...
        :param cfg2: HiluxNetworkConnectionCfg object to compare
    ...
    def __deepcopy__(self, memodict):
        newself = HiluxNetworkConnectionCfg(self.method)
    ...
    def is_http_proxy(self):
        ...
        proxy uses SSL while normal, "local" hiluxd does not. 
        """
        if self.__use_ssl:
            return True
        else:
