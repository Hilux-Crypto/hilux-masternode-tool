...
from hilux_utils import bip32_path_string_to_n, pubkey_to_address, bip32_path_n_to_string, bip32_path_string_append_elem
...
class Bip44Wallet(QObject):
    ...
    def __init__(self, coin_name: str, hw_session: 'HwSessionInfo', db_intf: DBCache, hiluxd_intf: HiluxdInterface,
                 hilux_network: str):
    ...
    def get_block_height(self):
        if self.cur_block_height is None or \
           (time.time() - self.last_get_block_height_ts >= GET_BLOCKHEIGHT_MIN_SECONDS):
            new_bh = self.hiluxd_intf.getblockcount()
        ...
        if not addr:
            ...
            try:
                ...
                if not row:
                    ...
                    address = pubkey_to_address(key.PublicKey().hex(), self.hilux_network)
    ...
    def _get_account_by_index(self, account_index: int, db_cursor) -> Bip44AccountType:
        ...
        if not account:
            ...
            if row:
                ...
                account.evaluate_address_if_null(db_cursor, self.hilux_network)
    ...
    def _process_addresses_txs(self, addr_info_list: List[Bip44AddressType], max_block_height: int,
        ...
        try:
            ...
            if last_block_height < max_block_height:
                ...
                txids = self.hiluxd_intf.getaddressdeltas({'addresses': addresses,
            ...
            try:
                mempool_entries = self.hiluxd_intf.getaddressmempool(addresses)
            ...
            for a in addr_info_list:
                if time.time() - a.last_balance_verify_ts >= ADDR_BALANCE_CONSISTENCY_CHECK_SECONDS:
                    ...
                    try:
                        b = self.hiluxd_intf.getaddressbalance([a.address])
                        bal = b.get('balance')
                        ...
                        if bal is not None and bal != a.balance:
                            log.warning('Balance of address %s inconsistency. Trying to refetch transactions.',
                                        a.id)
                            txids = self.hiluxd_intf.getaddressdeltas({'addresses': [a.address],
                                                                      'start': 0,
                                                                      'end': max_block_height})
    ...
    def _getrawtransaction(self, txhash, refetch_from_network: bool = False):
        ...
        tx = self.hiluxd_intf.getrawtransaction(txhash, 1)
    ...
    def _get_tx_db_id(self, db_cursor, txhash: str, tx_json: Dict = None, create=True) -> Tuple[int, Optional[Dict]]:
        ...
        if not row:
            if create:
                ...
                if block_height:
                    block_hash = self.hiluxd_intf.getblockhash(block_height)
                    block_header = self.hiluxd_intf.getblockheader(block_hash)
    ...
    def fetch_all_accounts_txs(self, check_break_process_fun: Callable, priority: int = DEFAULT_TX_FETCH_PRIORITY):
        ...
        def scan_account_txs(account, db_cursor):
            for change in (0, 1):
                ...
                change_level_node.evaluate_address_if_null(db_cursor, self.hilux_network)
