...
class MyTxApiInsight(TxApiInsight):
    ...
    def __init__(self, network, url, hiluxd_inf, cache_dir, zcash=None):
    ...
    def fetch_json(self, url, resource, resourceid):
        if resource == 'tx':
            try:
                j = self.hiluxd_inf.getrawtransaction(resourceid, 1, skip_cache=self.skip_cache)
                return j
            except Exception:
                raise
        else:
    ...
    def get_tx(self, txhash):
        ...
        if t.version == 3 and dip2_type != 0:
            ...
            t.extra_data = hilux_utils.num_to_varint(data["extraPayloadSize"]) + bytes.fromhex(
                data["extraPayload"]
            )
...
def sign_tx(hw_session: HWSessionBase, rt_data: AppRuntimeData, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    tx_api = MyTxApiInsight(insight_network, '', rt_data.hiluxd_intf, rt_data.tx_cache_dir)
