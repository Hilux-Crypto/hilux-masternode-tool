...
from hilux_utils import generate_bls_privkey, generate_wif_privkey, validate_address, wif_privkey_to_address, \
    validate_wif_privkey, bls_privkey_to_pubkey
from hiluxd_intf import HiluxdInterface
...
class RegMasternodeDlg(QDialog, QDetectThemeChange, ui_reg_masternode_dlg.Ui_RegMasternodeDlg, WndUtils):
    def __init__(self, main_dlg, config: AppConfig, hiluxd_intf: HiluxdInterface, masternode: MasternodeConfig,
        ...
        self.hiluxd_intf: HiluxdInterface = hiluxd_intf
        ...
        self.bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.main_dlg.hw_session,
                                        self.app_config.db_intf, self.hiluxd_intf, self.app_config.hilux_network)
    ...
    def update_dynamic_labels(self):
        ...
        def get_label_text(prefix:str, key_type: str, tooltip_anchor: str, style: str):
            lbl = prefix + ' ' + \
                  {'privkey': 'private key', 'pubkey': 'public key', 'address': 'Hilux address'}.get(key_type, '???')
        ...
        if self.masternode:
            ...
            if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                ...
                key_type, tooltip_anchor, placeholder_text = ('address', 'privkey', 'Enter the owner Hilux address')
                style = 'hl1'
            ...
            if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                ...
                key_type, tooltip_anchor, placeholder_text = ('address', 'privkey', 'Enter the voting Hilux address')
                style = 'hl1'
    ...
    def on_lblOwnerKey_linkHovered(self, link):
        if link == 'address':
            tt = 'Change input type to Hilux address'
        else:
    ...
    def get_network_info(self):
        try:
            info = self.hiluxd_intf.getnetworkinfo()
    ...
    def prepare_keys(self):
        ...
        protx_state = {}
        try:
            for protx in self.hiluxd_intf.protx('list', 'registered', True):
        ...
        if found_protx:
            if self.masternode.get_dmn_owner_public_address(self.app_config.hilux_network) == \
            ...
            if self.masternode.get_dmn_voting_public_address(self.app_config.hilux_network) == \
        ...
        if gen_owner:
            self.owner_pkey_generated =  generate_wif_privkey(self.app_config.hilux_network, compressed=True)
        ...
        if self.deterministic_mns_spork_active:
            if gen_voting:
                self.voting_pkey_generated = generate_wif_privkey(self.app_config.hilux_network, compressed=True)
    ...
    def on_btnGenerateOwnerKey_clicked(self, active):
        k = generate_wif_privkey(self.app_config.hilux_network, compressed=True)
            ...
            if self.show_field_hinds:
                if self.edtIP.text().strip():
                    ...
                    msg = 'If don\'t set the IP address and port fields, the masternode operator will ' \
                          'have to issue a ProUpServTx transaction using Hilux wallet.'
                    style = 'warning'
    ...
    def upd_payout_addr_info(self, show_invalid_data_msg: bool):
        ...
        if show_invalid_data_msg and self.payout_address_validation_err_msg:
            ...
            if self.show_field_hinds:
                msg = 'The owner\'s payout address can be set to any valid Hilux address - it no longer ' \
        ...
        if show_invalid_data_msg and self.owner_key_validation_err_msg:
            ...
            if self.show_field_hinds:
                if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                    ...
                    msg = 'You can use Hilux address if the related private key is stored elsewhere, eg in ' \
                          'the Hilux Core wallet.<br><span class="warning">Note, that if you provide an address ' \
            ...
            if show_invalid_data_msg and self.voting_key_validation_err_msg:
                ...
                if self.show_field_hinds:
                    if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                        ...
                        msg = 'You can use Hilux address if the related private key is stored elsewhere, eg in ' \
                              'the Hilux Core wallet.<br><span class="warning">Note, that providing an address instead of ' \
    ...
    def get_hilux_node_type(self):
        if self.rbDMTHiluxNodeType.isChecked():
            return NODE_TYPE_PUBLIC_RPC
        elif self.rbOwnHiluxNodeType.isChecked():
            return NODE_TYPE_OWN
        else:
            return None

    def upd_node_type_info(self):
        nt = self.get_hilux_node_type()
        msg = ''
        if nt is None:
            msg = 'DIP-3 masternode registration involves sending a special transaction via the v0.13 Hilux node ' \
                  '(eg Hilux-Qt). <b>Note, that this requires incurring a certain transaction fee, as with any ' \
            ...
            msg = 'The ProRegTx transaction will be processed via the remote RPC node stored in the app configuration.' \
                  ...
                  'so use this method only if you trust the operator of that node (nodes <i>alice(luna, suzy).hilux-masternode-tool.org</i> are maintained by the author of this application).<br><br>' \
                  '<b>Note 2:</b> if the operation fails (e.g. due to a lack of funds), choose the manual method ' \
                  'using your own Hilux wallet.'

        elif nt == NODE_TYPE_OWN:
            msg = 'A Hilux Core wallet (v0.13) with sufficient funds to cover transaction fees is required to ' \
                  'complete the next steps.'
        self.lblHiluxNodeTypeMessage.setText(msg)
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            if self.dmn_operator_key_type == InputKeyType.PRIVATE:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>hilux.conf</code> file on your masternode server ' \
                                   '(and restart <i>hiluxd</i>) or pass it to the masternode operator:'
            else:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>hilux.conf</code> file on your masternode server, replacing ' \
    ...
    def validate_data(self):
        ...
        if not addr:
            ...
            if not validate_address(self.dmn_owner_payout_addr, self.app_config.hilux_network):
        ...
        if not key:
            ...
            if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                ...
                if not validate_wif_privkey(self.dmn_owner_privkey, self.app_config.hilux_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner private key.'
                else:
                    self.dmn_owner_address = wif_privkey_to_address(self.dmn_owner_privkey, self.app_config.hilux_network)
            else:
                self.dmn_owner_address = key
                self.dmn_owner_privkey = ''
                if not validate_address(self.dmn_owner_address, self.app_config.hilux_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner Hilux address.'
        ...
        if self.deterministic_mns_spork_active:
            ...
            if not key:
                ...
                if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                    ...
                    if not validate_wif_privkey(self.dmn_voting_privkey, self.app_config.hilux_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting private key.'
                    else:
                        self.dmn_voting_address = wif_privkey_to_address(self.dmn_voting_privkey, self.app_config.hilux_network)
                else:
                    self.dmn_voting_address = key
                    self.dmn_voting_privkey = ''
                    if not validate_address(self.dmn_voting_address, self.app_config.hilux_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting Hilux address.'
        else:
    ...
    def get_collateral_tx_address_thread(self, ctrl: CtrlObject, check_break_scanning_ext: Callable[[], bool]):
        ...
        try:
            tx = self.hiluxd_intf.getrawtransaction(self.dmn_collateral_tx, 1, skip_cache=True)
        except Exception as e:
        ...
        if vouts:
            if self.dmn_collateral_tx_index < len(vouts):
                ...
                if not ads or len(ads) < 0:
                    raise Exception('The collateral transaction output doesn\'t have the Hilux address assigned.')
                if vout.get('valueSat') != 1000e8:
                    raise Exception('The value of the collateral transaction output is not equal to 1000 Hilux.')
        ...
        if self.dmn_collateral_tx_address_path:
            ...
            msg = 'Looking for a BIP32 path of the Hilux address related to the masternode collateral.<br>' \
    ...
    def next_step(self):
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            if self.get_hilux_node_type() == NODE_TYPE_PUBLIC_RPC:
                cs = STEP_AUTOMATIC_RPC_NODE
            elif self.get_hilux_node_type() == NODE_TYPE_OWN:
    ...
    @pyqtSlot(bool)
    def on_rbDMTHiluxNodeType_toggled(self, active):
        try:
            if active:
                self.upd_node_type_info()
        except Exception as e:
    ...
    @pyqtSlot(bool)
    def on_rbOwnHiluxNodeType_toggled(self, active):
        try:
            if active:
                self.upd_node_type_info()
        except Exception as e:
    ...
    def proregtx_automatic_thread(self, ctrl):
        ...
        try:
            ...
            try:
                mn_reg_support = self.hiluxd_intf.checkfeaturesupport('protx_register', self.app_config.app_version)
            ...
            try:
                ...
                if not public_proxy_node:
                    try:
                        ...
                        balances = self.hiluxd_intf.listaddressbalances(min_fee)
                        bal_list = []
                ...
                if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                    if self.v016_network:
                        ...
                        owner_key = wif_privkey_to_address(self.dmn_owner_privkey, self.app_config.hilux_network)
                else:
                ...
                call_ret = self.hiluxd_intf.rpc_call(True, False, 'protx', *tuple(params))
            ...
            try:
                self.dmn_reg_tx_hash = self.hiluxd_intf.rpc_call(True, False, 'protx', 'register_submit', protx_tx,
                                                                payload_sig_str)
    ...
    def update_manual_protx_prepare_command(self):
        ...
        if addr:
            valid = validate_address(addr, self.app_config.hilux_network)
            if valid:
                if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                    if self.v016_network:
                        owner_key = self.dmn_owner_privkey
    ...
    def check_tx_confirmation(self):
        try:
            tx = self.hiluxd_intf.getrawtransaction(self.dmn_reg_tx_hash, 1, skip_cache=True)
