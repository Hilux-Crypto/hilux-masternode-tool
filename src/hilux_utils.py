...
def get_chain_params(hilux_network: str) -> typing.ClassVar[ChainParams]:
    if hilux_network == 'MAINNET':
        return ChainParamsMainNet
    elif hilux_network == 'TESTNET':
        return ChainParamsTestNet
    else:
        raise Exception('Invalid \'network\' value.')


def get_default_bip32_path(hilux_network: str):
    return bip32_path_n_to_string([44 + 0x80000000, get_chain_params(hilux_network).BIP44_COIN_TYPE + 0x80000000, 0x80000000, 0, 0])


def get_default_bip32_base_path(hilux_network: str):
    return bip32_path_n_to_string([44 + 0x80000000, get_chain_params(hilux_network).BIP44_COIN_TYPE + 0x80000000])


def get_default_bip32_base_path_n(hilux_network: str):
    return [44 + 0x80000000, get_chain_params(hilux_network).BIP44_COIN_TYPE + 0x80000000]
...
def pubkey_to_address(pub_key, hilux_network: str):
    """Convert public key to a Hilux address."""
    pubkey_bin = bytes.fromhex(pub_key)
    pub_hash = bitcoin.bin_hash160(pubkey_bin)
    data = bytes([get_chain_params(hilux_network).PREFIX_PUBKEY_ADDRESS]) + pub_hash
...
def wif_privkey_to_address(privkey: str, hilux_network: str):
    pubkey = wif_privkey_to_pubkey(privkey)
    return pubkey_to_address(pubkey, hilux_network)


def validate_address(address: str, hilux_network: typing.Optional[str] = None) -> bool:
    """Validates if the 'address' is a valid Hilux address.
    :address: address to be validated
    :hilux_network: the hilux network type against which the address will be validated; if the value is None, then
    ...
    try:
        ...
        if len(data) > 5:
            prefix = data[0]
            if hilux_network:
                prefix_valid = (prefix == get_chain_params(hilux_network).PREFIX_PUBKEY_ADDRESS or
                                prefix == get_chain_params(hilux_network).PREFIX_SCRIPT_ADDRESS)
            else:
...
def generate_wif_privkey(hilux_network: str, compressed: bool = False):
    """
    ...
    if compressed:
        privkey += '01'
    data = bytes([get_chain_params(hilux_network).PREFIX_SECRET_KEY]) + bytes.fromhex(privkey)
    checksum = bitcoin.bin_dbl_sha256(data)[0:4]
    return base58.b58encode(data + checksum)


def validate_wif_privkey(privkey: str, hilux_network: str) -> bool:
    try:
        data = base58.b58decode(privkey)
        if len(data) not in (37, 38):
            raise Exception('Invalid private key length')

        if data[0] != get_chain_params(hilux_network).PREFIX_SECRET_KEY:
...
def wif_to_privkey(wif_key: str, hilux_network: str):
    """
    Based on project: https://github.com/chaeplin/hiluxmnb with some changes related to usage of bitcoin library.
    """
    privkey_encoded = base58.b58decode(wif_key).hex()
    wif_prefix_cur = privkey_encoded[:2]
    wif_prefix_network = get_chain_params(hilux_network).PREFIX_SECRET_KEY
...
def from_string_to_bytes(a):
    """
    Based on project: https://github.com/chaeplin/hiluxmnb.
    """
...
def ecdsa_sign(msg: str, wif_priv_key: str, hilux_network: str):
    ...
    pubkey = bitcoin.privkey_to_pubkey(wif_to_privkey(wif_priv_key, hilux_network))
...
def ecdsa_sign_raw(msg_raw: bytes, wif_priv_key: str, hilux_network: str):
...
def compose_tx_locking_script(dest_address, hilux_network: str):
    """
    ...
    if dest_address[0] in get_chain_params(hilux_network).B58_PREFIXES_PUBKEY_ADDRESS:
        ...
        scr = OP_DUP + \
              ...
              OP_EQUALVERIFY + \
              OP_CHECKSIG
    elif dest_address[0] in get_chain_params(hilux_network).B58_PREFIXES_SCRIPT_ADDRESS:
...
def convert_hilux_xpub(xpub, dest_prefix: str):
...
class CMasternodePing(object):
    ...
    def sign_message(self, priv_key, hilux_network):
        ...
        r = ecdsa_sign(self.sig_message, priv_key, hilux_network)
        self.sig = base64.b64decode(r)
        return self.sig

    def sign(self, priv_key, hilux_network, is_spork6_active: bool):
        if is_spork6_active:
            hash = self.get_hash()
            r = ecdsa_sign_raw(hash, priv_key, hilux_network)
            self.sig = base64.b64decode(r)
        else:
            self.sig = self.sign_message(priv_key, hilux_network)
        return self.sig
...
class CMasternodeBroadcast(object):
    ...
    def sign(self, collateral_bip32_path: str, hw_sign_message_fun: typing.Callable, hw_session,
             mn_privkey_wif: str, hilux_network: str):

        self.mn_ping.sign(mn_privkey_wif, hilux_network, is_spork6_active=self.spork6_active)
