...
from hilux_utils import wif_privkey_to_address, generate_wif_privkey, generate_bls_privkey, validate_address, \
...
class UpdMnRegistrarDlg(QDialog, QDetectThemeChange, ui_upd_mn_registrar_dlg.Ui_UpdMnRegistrarDlg, WndUtils):
    ...
    def read_data_from_network(self):
        try:
            ...
            if not protx:
                try:
                    protx = self.hiluxd_intf.protx('info', self.dmn_protx_hash)
    ...
    def update_ctrls_state(self):
        ...
        self.edtPayoutAddress.setToolTip('Enter a new payout Hilux address.')
        ...
        if self.dmn_voting_key_type == InputKeyType.PRIVATE:
            ...
            key_type, tooltip_anchor, placeholder_text = ('address', 'privkey', 'Enter a new voting Hilux address.')
            style = 'hl1'
        ...
        self.lblMessage.setText('<span style="color:#ff6600">By clicking <span style="font-weight:800">&lt;Send '
                                ...
                                'read notes <a href="https://github.com/Bertrand256/hilux-masternode-tool/blob/'
    ...
    def validate_data(self):
        ...
        if payout_address:
            if not validate_address(payout_address, self.app_config.hilux_network):
                raise Exception('Invalid payout Hilux address')
            else:
        ...
        if key:
            if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                ...
                if not validate_wif_privkey(self.dmn_new_voting_privkey, self.app_config.hilux_network):
                ...
                self.dmn_new_voting_privkey = ''
                if not validate_address(self.dmn_new_voting_address, self.app_config.hilux_network):
                    self.edtVotingKey.setFocus()
                    raise Exception('Invalid voting Hilux address.')
        else:
    ...
    def update_manual_cmd_info(self):
        try:
            ...
            if changed:
                ...
                msg = "<ol>" \
                      "<li>Start a Hilux Core wallet with sufficient funds to cover a transaction fee.</li>"
                msg += "<li>Import the owner private key into the Hilux Core wallet if you haven't done this " \
                       "before (<a href=\"https://github.com/Bertrand256/hilux-masternode-tool/blob/master/doc/" \
                ...
                msg += "<li>Execute the following command in the Hilux Core debug console:<br><br>"
    ...
    def on_btnSendUpdateTx_clicked(self, enabled):
        ...
        if self.dmn_prev_payout_address == self.dmn_new_payout_address and \
            ...
            WndUtils.warn_msg('Nothing is changed compared to the data stored in the Hilux network.')
        else:
            self.send_upd_tx()
    ...
    def send_upd_tx(self):
        ...
        try:
            ...
            try:
                upd_reg_support = self.hiluxd_intf.checkfeaturesupport('protx_update_registrar',
            ...
            upd_tx_hash = self.hiluxd_intf.rpc_call(True, False, 'protx', *params)
