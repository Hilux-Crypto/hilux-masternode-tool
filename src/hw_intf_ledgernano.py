...
class btchipDMT(btchip):
    ...
    def getTrustedInput(self, transaction, index):
        """This is an original method from btchip.btchip class adapted to support Hilux DIP2 extra data.
        ...
        if transaction.extra_data:
            # Hilux DIP2 extra data: By appending data to the 'lockTime' transfer we force the device into the
    ...
    def process_ledger_exceptions_int(*args, **kwargs):
        try:
            ...
            if (e.sw in (0x6d00, 0x6700)):
                e.message += '\n\nMake sure the Hilux app is running on your Ledger device.'
            elif (e.sw == 0x6982):
...
def open_session(dongle: HIDDongleHIDAPI):
    ...
    try:
        ...
        # exception would occur if the user haven't run the Hilux app on the device
        dongle.close()
        raise
...
def _ledger_extract_address(addr: str) -> str:
    ...
    if match and len(match.groups()) == 1:
        addr = match.group(1)
    elif not hilux_utils.validate_address(addr):
        raise Exception("Invalid Hilux address format returned from getWalletPublicKey: " + addr)
    return addr
...
def sign_tx(hw_session: HWSessionBase, rt_data: AppRuntimeData, utxos_to_spend: List[wallet_common.UtxoType],
    ...
    for utxo in utxos_to_spend:
        if utxo.txid not in rawtransactions:
            tx = rt_data.hiluxd_intf.getrawtransaction(utxo.txid, 1, skip_cache=False)
            if tx and tx.get('hex'):
                tx_raw = tx.get('hex')
            else:
                tx_raw = rt_data.hiluxd_intf.getrawtransaction(utxo.txid, 0, skip_cache=False)
    ...
    for idx, utxo in enumerate(utxos_to_spend):
        ...
        if data['version'] == 3 and dip2_type != 0:
            ...
            prev_transaction.extra_data = hilux_utils.num_to_varint(data["extraPayloadSize"]) + bytes.fromhex(
                data["extraPayload"])
        else:
    ...
    for out in tx_outputs:
        ...
        output.script = compose_tx_locking_script(out.address, rt_data.hilux_network)
