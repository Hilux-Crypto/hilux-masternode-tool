...
class WalletDlg(QDialog, ui_wallet_dlg.Ui_WalletDlg, WndUtils):
    ...
    def __init__(self, main_ui, hw_session: HwSessionInfo, initial_mn_sel: int):
        ...
        self.hiluxd_intf: HiluxdInterface = main_ui.hiluxd_intf
        self.db_intf: DBCache = main_ui.app_config.db_intf
        self.bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.hw_session, self.db_intf, self.hiluxd_intf,
    ...
    def restore_cache_settings(self):
        ...
        # base bip32 path:
        path = app_cache.get_value(CACHE_ITEM_HW_ACCOUNT_BASE_PATH.replace('%NETWORK%', self.app_config.hilux_network),
                                   hilux_utils.get_default_bip32_base_path(self.app_config.hilux_network), str)
        if hilux_utils.validate_bip32_path(path):
            self.hw_account_base_bip32_path = path
        else:
            self.hw_account_base_bip32_path = hilux_utils.get_default_bip32_base_path(self.app_config.hilux_network)
        ...
        if self.initial_mn_sel is None:
            sel_hashes = app_cache.get_value(
                CACHE_ITEM_UTXO_SRC_MASTRNODE.replace('%NETWORK%', self.app_config.hilux_network), [], list)
        ...
        enc_json_str = app_cache.get_value(CACHE_ITEM_LAST_RECIPIENTS.replace('%NETWORK%', self.app_config.hilux_network), None, str)
        if enc_json_str:
            try:
    ...
    def save_cache_settings(self):
        ...
        app_cache.set_value(CACHE_ITEM_HW_ACCOUNT_BASE_PATH.replace('%NETWORK%', self.app_config.hilux_network),
                            self.hw_account_base_bip32_path)
        ...
        for mna in self.selected_mns:
            if mna.address.address:
                ...
                sel_hashes.append(h)
        app_cache.set_value(CACHE_ITEM_UTXO_SRC_MASTRNODE.replace('%NETWORK%', self.app_config.hilux_network),
                            sel_hashes)
        ...
        app_cache.set_value(CACHE_ITEM_LAST_RECIPIENTS.replace('%NETWORK%', self.app_config.hilux_network), rcp_data)
    ...
    def on_btnSend_clicked(self):
        """
        Sends funds to Hilux address specified by user.
        """
        try:
            self.allow_fetch_transactions = False
            self.enable_synch_with_main_thread = False
            ...
            if len(tx_inputs):
                ...
                #  - the utxo Hilux (signing) address matches the hardware wallet address for a given path
                for utxo_idx, utxo in enumerate(tx_inputs):
                    ...
                    if utxo.is_collateral:
                        if self.query_dlg(
                                "Warning: you are going to transfer masternode's collateral (1000 Hilux) transaction "
                    ...
                    if addr_hw != utxo.address:
                        self.error_msg("<html style=\"font-weight:normal\">Hilux address inconsistency between UTXO "
                ...
                try:
                    ...
                    if tx_outputs:
                        ...
                        if len(tx_hex) > 90000:
                            ...
                            tx_dlg = TransactionDlg(self, self.main_ui.app_config, self.hiluxd_intf, tx_hex,
...
<tr><td class="lbl">Balance</td><td>{app_utils.to_string(balance/1e8)} Hilux</td></tr>
<tr><td class="lbl">Received</td><td>{app_utils.to_string(received/1e8)} Hilux</td></tr>
</table>
</body>
"""
