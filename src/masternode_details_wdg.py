...
class WdgMasternodeDetails(QWidget, ui_masternode_details_wdg.Ui_WdgMasternodeDetails):
    ...
    def setupUi(self, widget: QWidget):
        ...
        self.act_view_as_owner_public_address = QAction('View as Hilux address', self)
        ...
        self.act_view_as_voting_public_address = QAction('View as Hilux address', self)
                    ...
                    if self.masternode.dmn_owner_key_type == InputKeyType.PRIVATE:
                        if self.act_view_as_owner_private_key.isChecked():
                            ...
                            if self.masternode.dmn_owner_private_key:
                                ret = hilux_utils.wif_privkey_to_address(self.masternode.dmn_owner_private_key,
                                                                        self.app_config.hilux_network)
                        elif self.act_view_as_owner_public_key.isChecked():
                            if self.masternode.dmn_owner_private_key:
                                ret = hilux_utils.wif_privkey_to_pubkey(self.masternode.dmn_owner_private_key)
                        elif self.act_view_as_owner_public_key_hash.isChecked():
                            if self.masternode.dmn_owner_private_key:
                                pubkey = hilux_utils.wif_privkey_to_pubkey(self.masternode.dmn_owner_private_key)
                    ...
                    if self.masternode.dmn_voting_key_type == InputKeyType.PRIVATE:
                        if self.act_view_as_voting_private_key.isChecked():
                            ...
                            if self.masternode.dmn_voting_private_key:
                                ret = hilux_utils.wif_privkey_to_address(self.masternode.dmn_voting_private_key,
                                                                        self.app_config.hilux_network)
                        elif self.act_view_as_voting_public_key.isChecked():
                            if self.masternode.dmn_voting_private_key:
                                ret = hilux_utils.wif_privkey_to_pubkey(self.masternode.dmn_voting_private_key)
                        elif self.act_view_as_voting_public_key_hash.isChecked():
                            if self.masternode.dmn_voting_private_key:
                                pubkey = hilux_utils.wif_privkey_to_pubkey(self.masternode.dmn_voting_private_key)
                                pubkey_bin = bytes.fromhex(pubkey)
                                pub_hash = bitcoin.bin_hash160(pubkey_bin)
                                ret = pub_hash.hex()
                        else:
    ...
    def validate_keys(self):
        ...
        if self.masternode:
            if self.masternode.dmn_owner_key_type == InputKeyType.PRIVATE:
                if self.masternode.dmn_owner_private_key:
                    self.owner_key_invalid = not hilux_utils.validate_wif_privkey(self.masternode.dmn_owner_private_key,
                                                                                 self.app_config.hilux_network)
            else:
                if self.masternode.dmn_owner_address:
                    self.owner_key_invalid = not hilux_utils.validate_address(self.masternode.dmn_owner_address,
                                                                             self.app_config.hilux_network)

            if self.masternode.dmn_operator_key_type == InputKeyType.PRIVATE:
                if self.masternode.dmn_operator_private_key:
                    self.operator_key_invalid = not hilux_utils.validate_bls_privkey(
                        self.masternode.dmn_operator_private_key)
            else:
                if self.masternode.dmn_operator_public_key:
                    self.operator_key_invalid = not hilux_utils.validate_bls_pubkey(
                        self.masternode.dmn_operator_public_key)

            if self.masternode.dmn_voting_key_type == InputKeyType.PRIVATE:
                if self.masternode.dmn_voting_private_key:
                    self.voting_key_invalid = not hilux_utils.validate_wif_privkey(self.masternode.dmn_voting_private_key,
                                                                                  self.app_config.hilux_network)
            else:
                if self.masternode.dmn_voting_address:
                    self.voting_key_invalid = not hilux_utils.validate_address(self.masternode.dmn_voting_address,
                                                                              self.app_config.hilux_network)
    ...
    def generate_priv_key(self, pk_type: str, edit_control: QLineEdit, compressed: bool):
        ...
        if pk_type == 'operator':
            pk = hilux_utils.generate_bls_privkey()
        else:
            pk = hilux_utils.generate_wif_privkey(self.app_config.hilux_network, compressed=compressed)
        ...
        try:
            ...
            if utxos:
                ...
                if utxos is not None:
                    WndUtils.warn_msg('Couldn\'t find any 1000 Hilux UTXO in your wallet.')
        except Exception as e:
    ...
    def get_collateral_tx_address_thread(self, ctrl: CtrlObject,
        ...
        break_scanning = False
        txes_cnt = 0
        msg = 'Scanning wallet transactions for 1000 Hilux UTXOs.<br>' \
