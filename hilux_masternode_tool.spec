...
# source: https://github.com/akhavr/electrum-hilux/blob/master/contrib/osx/osx.spec
hiddenimports = []
...
a = Analysis(['src/hilux_masternode_tool.py'],
             pathex=[base_dir],
...
if os_type == 'win32':
    ...
    os.system('"7z.exe" a %s %s -mx0' % (
        os.path.join(all_bin_dir, 'HiluxMasternodeTool_' + version_str + '.win' + no_bits + '.zip'),
        'HiluxMasternodeTool.exe'))
elif os_type == 'darwin':
    print('Compressing Mac executable')
    os.system('zip -r "%s" "%s"' % (
        os.path.join(all_bin_dir, 'HiluxMasternodeTool_' + version_str + '.mac.zip'), 'HiluxMasternodeTool.app'))
elif os_type == 'linux':
    print('Compressing Linux executable')
    os.system('tar -zcvf %s %s' % (
        os.path.join(all_bin_dir, 'HiluxMasternodeTool_' + version_str + '.linux.tar.gz'), 'HiluxMasternodeTool'))
