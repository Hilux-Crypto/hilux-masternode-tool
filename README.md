...
# Hilux Masternode Tool (DMT)

## Contents

 * [Masternodes](#masternodes)
 * [Hilux Masternode Tool](#hilux-masternode-tool)
 ...
 * [Configuration](#configuration)
   ...
   * [Masternode setup](#masternode-setup)
     * [Scenario A: moving masternode management from Hilux Core](doc/config-masternodes-a.md)
 ...
 * [Downloads](https://github.com/Bertrand256/hilux-masternode-tool/releases/latest)
 * [Changelog](changelog.md)

## Masternodes

Hilux masternodes are full nodes which are incentivized by receiving a share of the block reward as payment in return 
...
software, you must dedicate 1000 Hilux as *collateral*, which is *"tied up"* in your node as long as you want 
...
A server with the Hilux daemon software installed will operate as a Hilux full node, but before the rest of the 
network accepts it as a legitimate masternode, one more thing must happen: the person controlling the node 
must prove that they are also in control of the private key to the node's 1000 Hilux *collateral*. 
This is achieved by sending a special transaction to the network (ProRegTx), signed by this private key.

This action can be carried out using the *Hilux Core* reference software client. As can be expected, this 
requires sending 1000 Hilux to an address controlled by the *Hilux Core* wallet. After the recent increase 
in the value of Hilux and a burst in the amount of malware distributed over the Internet, you do not have 
...
* Transferring masternode earnings safely, without touching the 1000 Hilux funding transaction
...
Most of the application features involve exchanging data between the application itself and the Hilux network. To do this, *DMT* needs to connect to one of the full nodes on the network, specifically one which can handle JSON-RPC requests. This node plays the role of a gateway for *DMT* to the Hilux network. It does not matter which full node provides the service, because all nodes reach consensus by synchronizing information between each other on the Hilux network.

Depending on your preferences (and skills) you can choose one of three possible connection types:
 * [Direct connection to a local node](doc/config-connection-direct.md), for example to *Hilux Core* running on your normal computer.
 * [Connection to a remote node through an SSH tunnel](doc/config-connection-ssh.md), if you want to work with a remote Hilux daemon (like your masternode) through an SSH tunnel.
...
Here we make the following assumptions:
  * You already have a server running the Hilux daemon software (*hiluxd*) that you want to use as a masternode. If you don't, you will need to install and configure one first by following the guide on the [Hilux Wiki](https://docs.hilux.org/en/stable/masternodes/setup.html).
  * We occasionally refer to the *hiluxd* configuration file, so it is assumed that *hiluxd* is running under a Linux operating system (OS), which is the most popular and recommended OS for this purpose.
  * Your server has a public IP address that will be visible on the Internet.
  * You have set up a TCP port on which your *hiluxd* listens for incoming connections (usually 9999).

Further configuration steps depend on whether you already have a masternode controlled by *Hilux Core* which you want to migrate to a hardware wallet managed by *DMT*, or if you are setting up a new masternode.
...
* `--config`: a non-standard path to a configuration file. Example:
  `HiluxMasternodeTool.exe --config=C:\dmt-configs\config1.ini`
...
Once you set up the Hilux daemon and perform the required *DMT* configuration, you need to broadcast the `start masternode` message to the Hilux network, so the other Hilux nodes recognize your daemon as a masternode and add it to the payment queue.
...
2. Opening a connection to the Hilux network and verifying if the Hilux daemon to which it is connected is not still waiting for synchronization to complete.
  Message in case of failure:  
  ![Hilux daemon synchronizing](doc/img/startmn-synchronize-warning.png)
...
6. Retrieving the Hilux address from the hardware wallet for the `BIP32 path` specified in the configuration. If it differs from the collateral address provided in the configuration, the following warning appears:  
  ![Hilux address mismatch](doc/img/startmn-addr-mismatch-warning.png)  
  The most common reason for this error is mistyping the hardware wallet passphrase. Remember that different passphrases result in different Hilux addresses for the same BIP32 path.

7. Verification that the specified transaction ID exists, points to your collateral address, is unspent and is equal to exactly 1000 Hilux. Messages in case of failure:  
  ![Could not find the specified transaction id](doc/img/startmn-tx-warning.png)  
  ![Collateral transaction output should equal 1000 Hilux](doc/img/startmn-collateral-warning.png)  
  If you decide to continue anyway, you probably won't be able to successfully start your masternode.

8. Verification at the Hilux network level that the specified transaction ID is valid. Message in case of failure:  
...
DMT version 0.9.4 and above allows you to transfer your masternode earnings. Unlike other Hilux wallets, DMT gives you a 100% control over which *unspent transaction outputs* (utxo) you want to transfer. This has the same effect as the `Coin control` functionality implemented in the *Hilux Core* wallet.

The `Transfer funds` window shows all *UTXOs* of the currently selected Masternode (mode 1), all Masternodes in current configuration (mode 2) or any address controlled by a hardware wallet (mode 3). All *UTXOs* not used as collateral are initially selected. All collateral *UTXOs* (1000 Hilux) are initially hidden to avoid unintentionally spending collateral funds and thus breaking MN. You can show these hidden entries by unchecking the `Hide collateral utxos` option.
...
After signing the transaction with your hardware wallet, the application will display a summary and will ask you for confirmation for broadcasting the signed transaction to the Hilux network.  
![Broadcast signed transaction confirmation](doc/img/dmt-transfer-funds-broadcast.png)

After clicking `Send Transaction`, the application broadcasts the transaction and then shows a confirmation with a transaction ID as a hyperlink directing to a Hilux block explorer:  
...
Binary versions of the latest release can be downloaded from: https://github.com/Bertrand256/hilux-masternode-tool/releases/latest.
...
2. Change the current directory to the folder where the DMT release files were downloaded:
   ...
   ```
   <username>: ls
   HiluxMasternodeTool_0.9.15.mac.zip    HiluxMasternodeTool_0.9.15.mac.zip.asc
   ```
...
3. Verify the signature by executing the following command:
   ...
   ```
   keybase pgp verify -d HiluxMasternodeTool_0.9.15.mac.zip.asc -i HiluxMasternodeTool_0.9.15.mac.zip -S bertrand256
   ```
